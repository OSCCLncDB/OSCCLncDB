import { useState } from 'react';
    ...
    export default function LncRNADetail({ lncrna }) {
      const [showSequence, setShowSequence] = useState(false);

      if (!lncrna) {
        return <Layout><div>lncRNA not found.</div></Layout>;
      }
    ...
    export async function getStaticPaths() {
      ...
      const lncrnas = JSON.parse(jsonData);

      const paths = lncrnas.map((lncrna) => ({
        params: { id: lncrna.id },
      }));
    ...
    export async function getStaticProps({ params }) {
      ...
      const lncrna = lncrnas.find((p) => p.id === params.id);
